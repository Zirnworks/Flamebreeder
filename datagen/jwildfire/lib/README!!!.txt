Welcome to JWildfire!

If you are new to the software and/or have problems with the launching,
please read the file README_LAUNCHING_JWILDFIRE.txt.

Please also read the file README_LINUX.txt if you have problems to start the program under Linux.

The software was successfully tested on various operating systems, including:
-Windows 7 - 10
-macOS High Sierra and Catalina
-Ubuntu Linux 16.04 LTS and 18.04 LTS

If you need additional support please read the file README_SUPPORT.txt

Have fun!

    